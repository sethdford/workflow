...
var workflow = {
    ...
    define: function(name, opts) {
        ...
        var workflowObj = {
            ...
            //** defines some workflow specific config/defaults
            config: _.assign({
                maxFailures: 5,
            ...
            verify: function() {
                ...
                function verifyTasks() {
                    var actions = _.map(workflowObj.tasks, function(obj) {
                        return obj.verify()
                    });
            ...
            getTask: function(name) {
                return _.find(this.tasks, function(act) {
        ...
        typeof(dfltTaskList) == 'string' && (workflowObj.swf.defaultTaskList = {
            name: dfltTaskList
        });
...
var activity = {
    ...
    complete: function(taskToken, result) {
        return this.svc.respondActivityTaskCompleted({
            taskToken: taskToken,
            result: result
        });
    },
    ...
    cancel: function(taskToken, details) {
        return this.svc.respondActivityTaskCancelled({
            taskToken: taskToken,
            details: details
        });
    },
    ...
    fail: function(taskToken, details, reason) {
        return this.svc.respondActivityTaskFailed({
            taskToken: taskToken,
            details: details,
            reason: reason
        });
...
_.extend(decisionContext.prototype, {
    ...
    isWorkflowComplete: function() {
        return !!_.find(this.decisions, function(dec) {
            return dec.decisionType == 'CompleteWorkflowExecution'
        });
    },

    isWorkflowFailed: function() {
        return !!_.find(this.decisions, function(dec) {
            return dec.decisionType == 'FailWorkflowExecution'
        });
    },

    isWorkflowCancelled: function() {
        return !!_.find(this.decisions, function(dec) {
            return dec.decisionType == 'CancelWorkflowExecution'
        });
    },
...
function eventHistory(events, opts) {
    this.events = events || [];
    this.options = opts || {};
    this.sorted = this.sortBy(function(evt) {
        return evt.eventTimestamp
    });
}
...
_.assign(eventHistory.prototype, {
    current: function() {
        return this.wrapEvent(this.sorted[this.sorted.length > 0 && this.sorted.length - 1 || 0]);
    },
    previous: function() {
        return this.wrapEvent(this.sorted[this.sorted.length - (this.sorted.length >= 2 ? 2 : 1)]);
    },
...
var workflowjs = module.exports = function(opt) {
    ...
    this.svc = {};
    this.defaultTaskList = (typeof(opt.defaultTaskList) === 'string' ? {
        name: opt.defaultTaskList
    } : opt.defaultTaskList);
    ...
    this.run = function(name, data, taskList, opts) {
        ...
        if (!wkflow) {
            ...
            var version = opts.version || wkflow.swf.version,
                taskList = (typeof(taskList) === 'string' ? {
                    name: taskList
                } : taskList) || wkflow.swf.defaultTaskList;
    ...
    this.verifyWorkflows = function() {
        ...
        var actions = _.map(_workflows, function(obj) {
            return obj.verify();
        });
        Q.all(actions)
            .then(def.resolve)
...
util.inherits(workflowjs, events.EventEmitter);